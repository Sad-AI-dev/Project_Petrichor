![[Purity.png|128]]

*Purity* is a **<span class="blue-bg">lunar</span>** item from Risk of Rain 2.
For all items, see the [[Items.canvas|Items]] page.

### <span class="lore-title-inline">Pickup Description</span>
<span class="lore-text">Reduce your skill cooldowns by 2 seconds. <span class="red">You are unlucky.</span></span>
# <span class="lore-title-inline">Lore</span>
<span class="lore">It looks like starseed, tanning in dirt.
It smells like hot stone, and tall grass.
It tastes like spiced fruit, sweet and hot.
It feels like solar winds, and solar chimes.
It sounds like two brothers, chasing glass frogs in the sun.</span>
# <span class="lore-title-inline">Notes</span>
Supposedly, the [[Purity]] resembles [[starseed]].

The spiced fruit, which tastes sweet and hot may be in reference to [[kifruit]].

The two brothers referred to are [[Providence]] and [[Mithrix]].

#### <span class="lore-text">Tags</span>
