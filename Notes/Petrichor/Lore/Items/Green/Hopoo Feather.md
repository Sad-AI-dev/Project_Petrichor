![[128px-Hopoo_Feather.webp|128]]

*Hopoo Feather* is a **<span class="green-bg">green</span>** item from Risk of Rain 2.
For all items, see the [[Items.canvas|Items]] page.

### <span class="lore-title-inline">Pickup Description</span>
<span class="lore-text">Gain an extra jump.</span>
# <span class="lore-title-inline">Lore</span>
<span class="lore">“...Europan laws have cracked down heavily on poaching, with the intent to help the reemergence of the hopoo. Since then, the Greebokks Foundation has helped support hopoo conservation groups, substantially growing their population beyond the initial three that were recorded in recent years.”

- Europan Wildlife Guide</span>
# <span class="lore-title-inline">Notes</span>
This log is a passage from the [[Europan Wildlife Guide]], covering the history of the `hopoo` is feathered creature native to [[Jupiter|Jupiter's]] moon [[Europa]].

The log introduces the `Greebokks Foundation`: an organization dedicated to assisting conservation efforts.
This is a reference to Gearbox helping the development studio grow beyond the initial 3 members.

# <span class="lore-title-inline">Lore</span>
<span class="lore"><span class="yellow">Shipping Details:</span>
  <span class="lore-text-dark">Tracking Number: 1412***********

Order Details:</span>
A feather from the hopoo, found only natively in Europa. Ever since its extreme light-weight vs tensile strength was discovered, the hopoo was almost driven to extinction from poaching. There are only 3 left; I hope you can keep their location safe.

<span class="lore-text-dark">Sent To:                                         Shipping Method:</span>
2341                                               <span class="green">Priority/Biological</span>
RKR Center                                    <span class="lore-text-dark">Estimated Delivery:</span>
Io                                                   07/03/2056</span>
# <span class="lore-title-inline">Notes</span>
This item is being shipped to [[Io]].

This log shows that the `hopoo` is a bird native to [[Europa]].
The fact that there are only 3 left is a reference to the size of the development team.
#### <span class="lore-text">Tags</span>
#shipping_2056