![[Evolvednematocystnozzle.png|128]]

*Evolved Nematocyst Nozzle* is a **<span class="yellow-bg">boss</span>** item from Risk of Rain Returns.
For all items, see the [[Items.canvas|Items]] page.

### <span class="lore-title-inline">Pickup Description</span>
<span class="lore-text">Cnidarian delicacy.</span>
# <span class="lore-title-inline">Lore</span>
<span class="lore"><span class="yellow">Shipping Details:</span>
  <span class="lore-text-dark">Tracking Number: 1585***********

Order Details:</span>
Across all the hills and valleys of the planet I've yet to see a single jellyfish of this hue. More passive than the others, I found this golden juvenile clinging onto some excess lump of flesh. It seems familiar, but the heat emanating from the mound is unlike anything I've come across. This is dangerous.

<span class="lore-text-dark">Sent To:                                         Shipping Method:</span>
8800,                                             <span class="yellow">Field-Found</span>
Gentle Valley,                                 <span class="lore-text-dark">Estimated Delivery:</span>
Unknown                                       12/31/2056</span>
# <span class="lore-title-inline">Notes</span>
The [[Evolved Nematocyst Nozzle]] can be retrieved from an evolved [[Young Jelly]].

The pickup description suggests a race or region called [[Cnidaria]], which is where the [[Jellyfish Family]] likely originates from.

The item appears to be an evolved version of the [[Nematocyst Nozzle]] and likely serves a similar purpose.

#### <span class="lore-text">Tags</span>
#shipping_2056